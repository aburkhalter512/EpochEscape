    ...
    IActivatable, IToggleable, ISerializable, IPlaceable, IConnectable
{
    ...
    public override void remove()
    {
        if (placed())
        {
            ...
            Utilities.floorTo(pos1, 2);
            Utilities.Vec2Int pos2 = null;

            for (int i = 0; i < mAttached.Length; i++)
            {
                pos2 = mAttached[i].position();
                Utilities.floorTo(pos2, 2);
            ...
            PlaceableStaticWall.createStaticWall(_map.toTilePos(pos1) + Utilities.toVector3(Map.tileSize) / 2);
            PlaceableStaticWall.createStaticWall(_map.toTilePos(pos2) + Utilities.toVector3(Map.tileSize) / 2);
        }
