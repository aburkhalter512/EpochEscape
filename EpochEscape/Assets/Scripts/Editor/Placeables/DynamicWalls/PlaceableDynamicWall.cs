...
public abstract class PlaceableDynamicWall : PlaceableObject, IPlaceable, ISerializable, IConnectable
{
    ...
    private Dictionary<SIDE, int> _armLengths;
    ...
    protected override void Awake()
    {
        ...
        //mEndCaps = new Dictionary<SIDE, Tile>();

        _armLengths = new Dictionary<SIDE, int>();
        _armLengths[SIDE.RIGHT] = 0;
        _armLengths[SIDE.TOP] = 0;
        _armLengths[SIDE.LEFT] = 0;
        _armLengths[SIDE.BOTTOM] = 0;
    ...
    public override IEnumerator serialize(XmlDocument doc, Action<XmlElement> callback)
    {
        ...
        int east = (mEndCaps[SIDE.RIGHT].position().x - basePos.x) / 2;*/
        dynamicwall.SetAttribute("east", _armLengths[SIDE.RIGHT].ToString());

        //int north = (mEndCaps[SIDE.TOP].position().y - basePos.y) / 2;
        dynamicwall.SetAttribute("north", _armLengths[SIDE.TOP].ToString());

        //int west = (basePos.x - mEndCaps[SIDE.LEFT].position().x) / 2;
        dynamicwall.SetAttribute("west", _armLengths[SIDE.LEFT].ToString());

        //int south = (basePos.y - mEndCaps[SIDE.BOTTOM].position().y) / 2;
        dynamicwall.SetAttribute("south", _armLengths[SIDE.BOTTOM].ToString());

        XmlElement boxCollider = doc.CreateElement("boxcollider2d");
        Vector2 colliderSize = Map.tileSize;
        colliderSize.x *= _armLengths[SIDE.RIGHT] + _armLengths[SIDE.LEFT] + 1;
        Vector2 colliderCenter = Vector2.zero;
        colliderCenter.y = (_armLengths[SIDE.TOP] - _armLengths[SIDE.BOTTOM]) * Map.tileSize.y;
        ...
        colliderSize.y *= _armLengths[SIDE.TOP] + _armLengths[SIDE.BOTTOM] + 1;
        colliderCenter = Vector2.zero;
        colliderCenter.x = (_armLengths[SIDE.RIGHT] - _armLengths[SIDE.LEFT]) * Map.tileSize.x;
        boxCollider.SetAttribute("size", colliderSize.ToString());
        boxCollider.SetAttribute("center", colliderCenter.ToString());
        dynamicwall.AppendChild(boxCollider);
    ...
    public override void moveTo(Vector3 v)
    {
        ...
        Vector3 offset = Utilities.toVector3(Map.tileSize / 2);
        if (v.x > 0)
            v.x += offset.x;
        else
            v.x -= offset.x;
    ...
    private void resize()
    {
        ...
        bool mIsAuxInput = _im.alternateInput.get();

        SIDE side = SIDE.TOP; // Just a dummy value
    ...
    private void setArea(SIDE side, int armLength, bool hasTiles)
    {
    ...
    private void updateTexture()
    {
        ...
        _sr.sprite = createFullTex(
            _armLengths[SIDE.RIGHT], 
            _armLengths[SIDE.TOP], 
            _armLengths[SIDE.LEFT], 
            _armLengths[SIDE.BOTTOM]);
    }
