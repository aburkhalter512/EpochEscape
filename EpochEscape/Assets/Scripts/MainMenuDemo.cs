...
public class MainMenuDemo : MonoBehaviour
{
    ...
    public enum Page{
		...
		SaveWarning, LoadWarning, DeleteWarning, SaveName, TutorialLevels, MainLevels, //Tutorial
	}
...
//	}
	...
	void OnGUI() {
		...
		switch (currentPage) {
			...
			case Page.TutorialLevels: ShowTutorialLevels(); break;
			case Page.MainLevels: ShowMainLevels(); break;
	...
	void ShowMain () {
		GUILayout.BeginArea (new Rect (500, 260, 400, 350));
			...
			if (GUILayout.Button ("Tutorial Levels", EpochSkin.GetStyle ("Top Button"))){
				ClickSound.Play ();	
				currentPage = Page.TutorialLevels;
			}
            G.getInstance().PlayHover(0);
			GUILayout.Space (10);
			if (GUILayout.Button ("Main Levels", EpochSkin.GetStyle ("Top Middle"))) {
					ClickSound.Play ();
                    currentBackdrop = Backdrops[0];
					characterNum = -1;
					currentPage = Page.MainLevels;
			}
...
//			GUILayout.EndHorizontal ();
        ...
        GUILayout.BeginArea(new Rect(775, 695, 110, 50));
			if (GUILayout.Button ("Select", EpochSkin.GetStyle ("Small Button"))) {
				...
				if(CharIsUnlocked ()){
					...
					G.getInstance().currentLevel = m_currentLevel;

					SceneManager.Load(m_levelName);
				}
			}
	...
	private void ShowTutorialLevels()
	{
		...
		G.getInstance().PlayHover(2);
		GUILayout.Space (10);
		if (GUILayout.Button ("Back", EpochSkin.GetStyle("Bottom Middle"))){
			ClickSound.Play ();
			currentPage = Page.Main;
		}
		G.getInstance().PlayHover(3);
