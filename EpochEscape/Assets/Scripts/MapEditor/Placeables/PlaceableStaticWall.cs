...
public class PlaceableStaticWall : PlaceableObject
{
    ...
    {
        GameObject prefab = Resources.Load<GameObject>("Prefabs/MapEditor/Placeable/StaticWall");

        PlaceableStaticWall staticWall = 
            (GameObject.Instantiate(prefab) as GameObject).GetComponent<PlaceableStaticWall>();

        staticWall.moveTo(v);
        if (!staticWall.place())
            GameObject.Destroy(staticWall);
    }
    ...
    public override void moveTo(Vector3 v)
    {
        ...
        if (!Utilities.IsApproximately(_oldPosition, transform.position))
        {
            updateTexture();
    ...
    public void updateTexture()
    {
        ...
        if (didUpdate || mIsFirstUpdate)
        {
            ...
            Texture2D tempTex = Utilities.subTexture(wallTex, 0, 0, texSize.x, texSize.y);
            bottomLeft.setTexture(tempTex);

            tempTex = Utilities.subTexture(wallTex, texSize.x, 0, texSize.x, texSize.y);
            bottomRight.setTexture(tempTex);//*/

            tempTex = Utilities.subTexture(wallTex, 0, texSize.y, texSize.x, texSize.y);
            topLeft.setTexture(tempTex);//*/

            tempTex = Utilities.subTexture(wallTex, texSize.x, texSize.y, texSize.x, texSize.y);
