...
public class StaticWallManager : ISerializable
{
    ...
    SortedList<Utilities.Vec2Int, PlaceableStaticWall> mStaticWalls;
    ...
    {
        mStaticWalls = new SortedList<Utilities.Vec2Int, PlaceableStaticWall>(Utilities.Vec2IntComparer.Get());
    ...
    public void add(PlaceableStaticWall wall)
    {
        ...
        Utilities.Vec2Int key = wall.bottomLeft.position();
    ...
    public IEnumerator serialize(XmlDocument doc, Action<XmlElement> callback)
    {
        ...
        foreach (KeyValuePair<Utilities.Vec2Int, PlaceableStaticWall> pair in mStaticWalls)
        {
            if (prevPos == null)
            {
                colliderHeight = 1;
