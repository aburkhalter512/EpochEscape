...
public class ChunkManager : Manager<ChunkManager>, ISerializable
{
    ...
    List<Utilities.Pair<Utilities.Vec2Int, Texture2D>> mQueuedTiles;
    bool mIsAwake = false;

    SortedList<Utilities.Vec2Int, Texture2D> mChunks;
    SortedList<Utilities.Vec2Int, Texture2D> mChunksToUpdate;

    Utilities.Vec2Int mChunkTexSize;
    Vector2 mChunkRealSize;

    Utilities.Vec2Int mTilesPerChunk = new Utilities.Vec2Int(16, 16);

    Utilities.Vec2Int mTileTexSize = new Utilities.Vec2Int(32, 32);
    ...
    protected override void Initialize()
    {
        ...
        mChunkTexSize = new Utilities.Vec2Int(
        ...
        mChunks = new SortedList<Utilities.Vec2Int, Texture2D>(Utilities.Vec2IntComparer.Get());
        mChunksToUpdate = new SortedList<Utilities.Vec2Int, Texture2D>(Utilities.Vec2IntComparer.Get());

        getChunk(new Utilities.Vec2Int(0, 0));
        getChunk(new Utilities.Vec2Int(0, -1));
        getChunk(new Utilities.Vec2Int(-1, 0));
        getChunk(new Utilities.Vec2Int(-1, -1));

        mIsAwake = true;

        if (mQueuedTiles == null)
            mQueuedTiles = new List<Utilities.Pair<Utilities.Vec2Int, Texture2D>>();

        foreach (Utilities.Pair<Utilities.Vec2Int, Texture2D> tile in mQueuedTiles)
    ...
    public void clearTileTexture(Utilities.Vec2Int logicalPosition)
    {
    ...
    public void setTileTexture(Utilities.Vec2Int logicalPosition, Texture2D tileTex)
    {
        ...
        if (!mIsAwake)
        {
            ...
            mQueuedTiles.Add(new Utilities.Pair<Utilities.Vec2Int, Texture2D>(new Utilities.Vec2Int(logicalPosition), tileTex));
            return;
        }
        ...
        Utilities.Vec2Int localTilePos = new Utilities.Vec2Int(0, 0);
    ...
    public Utilities.Vec2Int getTileTexSize()
    {
        return mTileTexSize;
    }
    ...
    public IEnumerator serialize(XmlDocument doc, Action<XmlElement> callback)
    {
        ...
        foreach (KeyValuePair<Utilities.Vec2Int, Texture2D> it in mChunks)
        {
    ...
    protected Texture2D getChunk(Utilities.Vec2Int logicalChunkPos)
    {
        Texture2D retVal;
        ...
        if (!mChunks.TryGetValue(logicalChunkPos, out retVal))
        {
            ...
            mChunks.Add(new Utilities.Vec2Int(logicalChunkPos), retVal);
        }
    ...
    protected Texture2D _createChunk(Utilities.Vec2Int logicalChunkPos)
    {
        ...
        Mesh chunkMesh = Utilities.makeQuadMesh(mChunkRealSize);
    ...
    protected void toChunk(
        Utilities.Vec2Int tilePos, 
        out Utilities.Vec2Int chunkPos, 
        out Utilities.Vec2Int localTilePos)
    {
        if (tilePos == null)
        {
            chunkPos = null;
        ...
        chunkPos = new Utilities.Vec2Int(0, 0);
        localTilePos = new Utilities.Vec2Int(0, 0);
