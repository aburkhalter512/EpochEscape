...
namespace Game
{
    public class HUDManager : Manager<HUDManager>
    {
        ...
        void OnGUI()
        {
            ...
            Matrix4x4 svMat = UnityEngine.GUI.matrix;
            UnityEngine.GUI.matrix = Matrix4x4.TRS(Vector3.zero, Quaternion.identity, scale);

            #region hero icon + detectionbar
            UnityEngine.GUI.BeginGroup(new Rect(iconPos.x, iconPos.y, iconSize.x + 200.0f, iconSize.y));
            UnityEngine.GUI.DrawTexture(new Rect(0f, 115f, iconSize.x, -95f * barFill), detectionBar);
            UnityEngine.GUI.DrawTexture(new Rect(0f, 0f, iconSize.x, iconSize.y), iconFrame);
            UnityEngine.GUI.DrawTexture(new Rect(30f, 21f, 100f, 100f), currMood);
            UnityEngine.GUI.DrawTexture(new Rect(20f, 120f, 30f, 30f), specItem);

            UnityEngine.GUI.EndGroup();
            #endregion

            #region power cores
            UnityEngine.GUI.BeginGroup(new Rect(corePos.x, corePos.y, jarSize.x, jarSize.y));
            UnityEngine.GUI.DrawTexture(new Rect(18f, 4f, coreSize.x, coreSize.y), currCore);
            UnityEngine.GUI.DrawTexture(new Rect(0f, 0f, jarSize.x, jarSize.y), coreJar);
            UnityEngine.GUI.EndGroup();
            #endregion

            updateTimer();

            UnityEngine.GUI.matrix = svMat;
        }
