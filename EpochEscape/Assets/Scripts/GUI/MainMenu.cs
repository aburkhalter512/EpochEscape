...
namespace Game
{
    public class MainMenu : MonoBehaviour
    {
        ...
        void ShowOptions()
        {
            ...
            G.Get().showFPS = GUILayout.Toggle(G.Get().showFPS, "", EpochSkin.toggle);
            GUILayout.EndHorizontal();

            //			GUILayout.BeginHorizontal ();
            //				GUILayout.Space (25);
            //				GUILayout.Label ("Tutorial On", EpochSkin.label);
            //				G.Get().Tutorial = GUILayout.Toggle (G.Get ().Tutorial, "", EpochSkin.toggle);
            //			GUILayout.EndHorizontal ();
            #endregion
        ...
        private void GraphicControl()
        {
            ...
            G.Get().graphicsQuality = (int)GUILayout.HorizontalSlider((float)G.Get().graphicsQuality, 1.0f, 6.0f, EpochSkin.horizontalSlider, EpochSkin.horizontalSliderThumb);
        ...
        void SelectMenu()
        {
            ...
            if (GUILayout.Button("Select", EpochSkin.GetStyle("Small Button")))
            {
                ...
                if (CharIsUnlocked())
                {
                    ...
                    G.Get().m_currentCharacter = characterNum;
                }
            }
            G.Get().PlayHover(6);
            GUILayout.EndArea();
        ...
        private bool CharIsUnlocked()
        {
            ...
            switch (characterNum)
            {
                ...
                case 1:
                    ...
                    break;
                case 2:
                    if (G.Get().ninjaUnlocked)
                        unlocked = true;
                    else
                        unlocked = false;
                    break;
                case 3:
                    if (G.Get().astroUnlocked)
                        unlocked = true;
                    else
                        unlocked = false;
                    break;
                case 4:
                    if (G.Get().mumUnlocked)
                        unlocked = true;
                    else
                        unlocked = false;
                    break;
                case 5:
                    if (G.Get().robUnlocked)
                        unlocked = true;
                    else
                        unlocked = false;
                    break;
        ...
        void ShowTypeName()
        {
            ...
            GUILayout.EndHorizontal();
            UnityEngine.GUI.SetNextControlName("SaveName");
            mName = GUILayout.TextField(mName.Replace("\n", "").Replace("\r", ""), 12, EpochSkin.textField);
            UnityEngine.GUI.FocusControl("SaveName");
            GUILayout.Space(10);
            GUILayout.BeginHorizontal();
        ...
        private string GenerateLevelName()
        {
            ...
            if (G.Get().currentLevel > MAX_LEVELS)
                G.Get().currentLevel = 1;
            levelName += G.Get().currentLevel.ToString();
            return levelName;
        }
        #endregion
